...
/**
 * Makes the wrapped {@link ValidatorTextMaskComponent} repeat zero or more times.
 */
final class ValidatorTextMaskComponentRepeating<T extends ValidationReference> extends ValidatorTextMaskComponent<T> {

    static <T extends ValidationReference> ValidatorTextMaskComponent<T> with(final ValidatorTextMaskComponent<T> component) {
        return new ValidatorTextMaskComponentRepeating<>(
            Objects.requireNonNull(component, "component")
        );
    }

    private ValidatorTextMaskComponentRepeating(final ValidatorTextMaskComponent<T> component) {
        super();
    ...
    ValidationErrorList<T> tryMatch(final TextCursor text,
        ...
        final ValidatorTextMaskComponent<T> component = this.component;
        ...
        while(text.isNotEmpty()) {
            ...
            final ValidationErrorList<T> errors = component.tryMatch(
                text,
                false, // invert
                Lists.of(ValidatorTextMaskComponentRepeatingNext.<T>instance()) // required otherwise nextComponent might consume text and fail.
                    .iterator(),
                context
            );
    ...
    public boolean equals(final Object other) {
        return this == other ||
            other instanceof ValidatorTextMaskComponentRepeating &&
                this.equals0(Cast.to(other));
    }

    private boolean equals0(final ValidatorTextMaskComponentRepeating<?> other) {
